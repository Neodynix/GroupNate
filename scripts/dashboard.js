...
// REPLACE WITH YOUR KEYS from Supabase Project -> Settings -> API
const supabaseUrl = 'https://zpoktahbfhnanizgvehh.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inpwb2t0YWhiZmhuYW5pemd2ZWhoIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MTc4OTA1MiwiZXhwIjoyMDg3MzY1MDUyfQ.vYBiB7vdZVFmpspreoqZ9aXPRc357tKQ_qEDJ0XBCZE';
...
async function fetchMyGroups() {
    ...
    // Update the active groups stat in the profile dropdown
...
function setupDashboardListeners() {
    subCountry.addEventListener('change', function() {
        ...
        if(locations[this.value]) {
            locations[this.value].forEach(city => subCity.appendChild(new Option(city, city)));
        }
    });
...
window.openPaymentModal = function(planName, price) {
    ...
    iframe.src = `https://example.com/checkout?plan=${planName}&amount=${price}`;
...
window.closeModals = function() {
    document.querySelectorAll('.overlay').forEach(modal => modal.classList.add('hidden'));
